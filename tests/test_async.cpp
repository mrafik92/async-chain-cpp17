...
struct MyService {
  ...
  static void final(Result<std::string, std::string> result) {
    if (result.is_ok()) {
...
TEST(AsyncChainTest, TwoStepsAndFinally) {
  ...
  initAsyncChain<int, std::string>().then(step1).then(step2).finally(finalStep);
...
TEST(AsyncChainTest, StructResultEachStepModifiesField) {
  ...
  MyData const initial{0, 0.0, ""};
  MyData final_data;
  bool final_ok = false;

  auto step1 = [](auto next, MyResult) {
    MyData const d{1, 0.0, ""};
    next(MyResult::Ok(d));
  };
...
TEST(AsyncChainDemo, ClassLambdas) {
  ...
  setScheduler([](const std::function<void()>& task, std::size_t) { task(); });
...
TEST(AsyncChainDemo, MultilevelChain) {
  ...
  auto step2 = [](auto next, Result<std::string, std::string>) {
    ...
    initAsyncChain<std::string, std::string>().then(s1).then(s2).finally(final);
  };
  auto finalStep = [&](Result<std::string, std::string> result) {
    if (result.is_ok()) {
      final_ok = true;
  ...
  setScheduler([](const std::function<void()>& task, std::size_t) { task(); });
  initAsyncChain<std::string, std::string>().then(step1).then(step2).finally(
      finalStep);
  EXPECT_FALSE(final_ok);
...
TEST(AsyncChainDemo, NestedChainWithErrorRecovery) {
  ...
  auto step2 = [](auto next, Result<std::string, std::string> /*result*/) {
    ...
    auto nested_final =
        [&next](Result<std::string, std::string> nested_result) {
          next(nested_result);
        };
    initAsyncChain<std::string, std::string>().then(nested1).finally(
        nested_final);
  };
  auto catcher = [](auto next, Result<std::string, std::string> error_result) {
    // Recover from nested error
    next(Result<std::string, std::string>::Ok("recovered from " +
                                              *error_result.error));
  };
  auto finalStep = [&](Result<std::string, std::string> result) {
    if (result.is_ok()) {
...
TEST(AsyncChainDemo, DeeplyNestedMultiLevelChain) {
  ...
  auto step1 = [](auto next, Result<std::string, std::string>) {
    ...
    auto nested1 = [](auto next, Result<std::string, std::string>) {
      ...
      initAsyncChain<std::string, std::string>().then(nested2).finally(
          nested2_final);
    };
  ...
  setScheduler([](const std::function<void()>& task, std::size_t) { task(); });
  initAsyncChain<std::string, std::string>().then(step1).finally(finalStep);
  EXPECT_TRUE(final_ok);
