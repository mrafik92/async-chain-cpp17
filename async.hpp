...
struct Holder {
  ...
  explicit Holder(Step &ref) : ptr(&ref) {
    static_assert(!std::is_reference_v<Step>, "Holder should not be used with reference types");
  }
  ...
  void call(Continue &&continue_chain, CurrentResult &&result) {
    ...
    (*ptr)([continue_chain = std::forward<Continue &&>(continue_chain)](auto next_result) mutable {
...
struct CatcherHolder {
  ...
  explicit CatcherHolder(Step &ref) : ptr(&ref) {
    static_assert(!std::is_reference_v<Step>, "CatcherHolder should not be used with reference types");
  }
  ...
  void call(Continue &&continue_chain, CurrentResult &&result) {
    if (result.is_err()) {
      (*ptr)(
        [continue_chain = std::forward<Continue &&>(continue_chain)](auto recovered_result) mutable {
...
struct RetryHolder {
  ...
  explicit RetryHolder(Step &ref) : ptr(&ref) {
    static_assert(!std::is_reference_v<Step>, "RetryHolder should not be used with reference types");
  }
...
private:
  ...
  void run_step(Continue &&continue_chain, std::size_t attempt) {
    (*ptr)(
      [continue_chain = std::forward<Continue>(continue_chain), this, attempt](auto result) mutable {
...
private:
  ...
  static void call_steps(StepsTuple &&steps, FinalCallback &&final_callback, CurrentResult &&result) {
    if constexpr (Index < std::tuple_size_v<std::decay_t<StepsTuple> >) {
      ...
      auto continue_chain = [steps = std::forward<StepsTuple>(steps), final_callback = std::forward<
            FinalCallback>(final_callback)](auto &&next_result) mutable {
        call_steps<Index + 1>(std::move(steps), std::move(final_callback),
                              std::forward<decltype(next_result)>(next_result));
      };


      holder.call(std::forward<decltype(continue_chain)>(continue_chain), std::forward<CurrentResult>(result));
    } else {
